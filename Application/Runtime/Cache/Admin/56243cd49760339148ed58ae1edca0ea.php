...
<html>
 <head>
	...
	<div id="content" class="white">
        <h1><img src="/api/Public/Admin/img/icons/posts.png" alt="" /> 积分商品管理-修改积分商品</h1>
        <div class="bloc">
		    <div class="title"><a href="/api/index.php?s=/Admin/CreditGoods/index">积分商品列表</a> &gt; <a href="/api/index.php?s=/Admin/CreditGoods/add">添加积分商品</a> &gt; <a href="/api/index.php?s=/Admin/CreditGoods/edit/id/<?php echo ($oneGoods["id"]); ?>">修改积分商品</a></div>
		    <form method="post" action="">
		    <input type="hidden" name="id" value="<?php echo ($oneGoods["id"]); ?>" />
		    <div class="content">
	    		<div class="input medium">商品标题：<input type="text" name="title" value="<?php echo ($oneGoods["title"]); ?>" /> <em class="red">*</em></div>
                <div class="input">商品积分：<input type="text" name="credit" value="<?php echo ($oneGoods["credit"]); ?>" /> <em class="red">*</em></div>
                <div class="input">商品库存：<input type="text" name="inventory" value="<?php echo ($oneGoods["inventory"]); ?>" /> <em class="red">*</em></div>
				<div class="input">缩略　图：
					<input type="text" name="thumbpic" id="thumbpic" value="<?php echo ($oneGoods["thumbpic"]); ?>" /> <em class="red">* <a href="javascript:;" class="button" onclick="openUp();">上传图片</a></em>
					<div id="thumb" style="padding: 10px 0 0 63px;"><img src="<?php echo ($oneGoods["thumbpic"]); ?>" alt="缩略图" /></div>
				</div>
		        <div class="input textarea">
		            <span class="middle">商品简介：</span><textarea name="descript" rows="3" cols="5"><?php echo ($oneGoods["descript"]); ?></textarea>
		        </div>
		        <div class="input textarea">
		        	商品详情：<textarea name="content" id="editor"><?php echo ($oneGoods["content"]); ?></textarea>
		        </div>
		        <div class="input">
		        	是否推荐：
		            <input type="radio" id="radio1" name="isrec" value="1" <?php if($oneGoods['isrec'] == 1): ?>checked="checked"<?php endif; ?> /><label for="radio1" class="inline">是</label>
		            <input type="radio" id="radio2"  name="isrec" value="0" <?php if($oneGoods['isrec'] == 0): ?>checked="checked"<?php endif; ?> /><label for="radio2" class="inline">否</label>
		        </div>
		        <div class="submit">
