...
<html>
 <head>
	...
	<div id="content" class="white">
        <h1><img src="/api/Public/Admin/img/icons/posts.png" alt="" /> 用户管理-编辑用户</h1>
        <div class="bloc">
		    <div class="title"><a href="/api/index.php?s=/Admin/Manage/index">用户列表</a> &gt; <a href="/api/index.php?s=/Admin/Manage/add">添加用户</a> &gt; <a href="javascript:;">编辑用户</a></div>
		    <form method="post" action="" enctype="multipart/form-data">
		    <input type="hidden" name="id" value="<?php echo ($oneManage["id"]); ?>" />
		    <div class="content">
	    		<div class="input">用户名称：<?php echo ($oneManage["username"]); ?></div>
		        ...
		        <div class="input">
		            ...
		            <?php if(is_array($grouplist)): $i = 0; $__LIST__ = $grouplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($oneManage['groupid'] == $vo['id']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["groupname"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		            </select> <em class="red">*</em>
		        </div>
		        <div class="input">
		            缩略　图：<input type="file" name="pic" /> <img src="<?php echo ($oneManage["head_img"]); ?>" class="thumb" alt="<?php echo ($oneManage["username"]); ?>" /> <em class="red">*</em>
		        </div>
		        <div class="input textarea">
		            <span class="middle">用户备注：</span><textarea name="remark" rows="3" cols="5"><?php echo ($oneManage["remark"]); ?></textarea>
		        </div>
		        <div class="input">
		        	用户状态：
		            <input type="radio" id="radio1" name="status" value="1" <?php if($oneManage['status'] == 1): ?>checked="checked"<?php endif; ?> /><label for="radio1" class="inline">启用</label>
		            <input type="radio" id="radio2"  name="status" value="0" <?php if($oneManage['status'] == 0): ?>checked="checked"<?php endif; ?> /><label for="radio2" class="inline">禁用</label>
		        </div>
		        <div class="submit">
