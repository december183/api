...
<html>
 <head>
				...
				<table>
		            ...
		            <tbody>
		            	<?php if(is_array($sellerlist)): $i = 0; $__LIST__ = $sellerlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		                    ...
		                    <td><?php if($vo['isrec'] == 1): ?><a href="javascript:;" onclick="setRec(<?php echo ($vo["id"]); ?>,this);">已推荐</a><?php else: ?><a href="javascript:;" onclick="setRec(<?php echo ($vo["id"]); ?>,this);">未推荐</a><?php endif; ?></td>
	...
	<script type="text/javascript">
		...
		function setRec(id,target){
			var $target=$(target);
			$.ajax({
				url:'/api/index.php?s=/Admin/Seller/setRec',
				data:{"id":id},
				type:'post',
				dataType:'json',
				success:function(response){
